...
else
{
	if( $action == '' || $action == 'list' )
	{
		...
		$xtpl->assign( 'LANG', $lang_module );
		$xtpl->assign( 'GLANG', $lang_global );
		$xtpl->assign( 'ADD_LINK', NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "/subject&amp;action=add" );
		...
		$base_url = NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "/subject&amp;search=1&amp;per_page=" . $search['per_page'] . "&amp;faculty_id=" . $search['faculty_id'] . "&amp;q=" . $search['q'];
		...
		if( $db->sql_numrows( $result ) > 0 )
		{
			...
			while( $row = $db->sql_fetchrow( $result ) )
			{
				...
				$row['url_edit'] = NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "/subject&amp;action=add&amp;subjectid=" . $row['subject_id'];
	...
	elseif( $action == 'add' )
	{
		...
		if( $subjectid == 0 )
		{
			$form_action = NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "/subject&amp;action=add";
		}
		else
		{
			...
			{
				Header( "Location: " . NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op . "/subject&action=add" );
				die();
			}
		
			$subject = $db->sql_fetchrow( $result );			
			$form_action = NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "/subject&amp;action=add&amp;subjectid=" . $subjectid;			
		}		
	}
}
